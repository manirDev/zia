document.addEventListener('DOMContentLoaded', function() {
    ...
    if (window.ziaInterpreter.setStatusCallback) {
        window.ziaInterpreter.setStatusCallback(function(message) {
            document.getElementById('status').textContent = message;
        });
    } else {
        console.error('ziaInterpreter not available');
        document.getElementById('status').textContent = 'Erreur: ziaInterpreter non disponible';
    }
    ...
    if (window.MonacoEditorLoader && window.MonacoEditorLoader.initEditor) {
        ...
        document.getElementById('status').textContent = 'Erreur: MonacoEditorLoader non disponible';
    }
    ...
    if (window.ziaInterpreter.loadModule) {
        window.ziaInterpreter.loadModule().then(function(success) {
            if (success) {
            ...
            document.getElementById('status').textContent = 'Erreur de chargement: ' + error.message;
        });
    }
    ...
    document.getElementById('darkmode-toggle').addEventListener('change', function() {
        ...
        if (window.MonacoEditorLoader && window.MonacoEditorLoader.toggleDarkMode) {
    ...
    document.addEventListener('click', function(e) {
        if (shortcutsVisible && !settingsButton.contains(e.target) && !document.getElementById('key-shortcuts-panel').contains(e.target)) {
            document.getElementById('key-shortcuts-panel').classList.remove('visible');
    ...
    loadExampleButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            const exampleCode = this.closest('.example-item').querySelector('.example-code').textContent;
            ...
            // Show notification
            showToast('Exemple chargé: ' + this.closest('.example-item').querySelector('.example-title span').textContent, 'success');
        });
    });
